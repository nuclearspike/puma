...
* **Increasing worker counts decreases memory per worker on average**. More processes per pod reduces memory usage per process, because of copy-on-write memory and because the cost of the single master process is "amortized" over more child processes. The `warmup_before_fork` setting (off by default; see [Preloading](../README.md)) can improve these copy-on-write savings further: it runs once in the master before the initial worker spawn, and -- in `fork_worker` mode -- once per refork in worker 0; it does not re-run for later individual worker respawns or scale-ups.
    * It performs a full GC and heap compaction, so any object that became unreachable during boot or your `before_fork`/`before_refork` hooks is collected (finalizers may run) immediately before the fork -- don't rely on GC timing in those hooks.
    * If your app already calls `Process.warmup` itself in a `before_fork` hook, leave `warmup_before_fork` off (or remove the hook) to avoid doing the work twice.
